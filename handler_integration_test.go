...
func (s *HandlerIntegrationTestSuite) TestToolHandlerIntegration() {
	s.Run("ToUpper", func() {
		...
		handler, err := mcpio.NewHandler(
			...
			mcpio.WithTool("to_upper", "Convert text to uppercase", func(ctx context.Context, toolCtx mcpio.ToolContext, input struct {
	...
	s.Run("CountWords", func() {
		...
		handler, err := mcpio.NewHandler(
			mcpio.WithName("text-processor"),
			mcpio.WithTool("count", "Count words or characters in text", func(ctx context.Context, toolCtx mcpio.ToolContext, input struct {
				Text string `json:"text"`
